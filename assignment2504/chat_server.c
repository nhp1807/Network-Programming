    ...
    while (1){
        ...
        if (FD_ISSET(server, &readfds)){
            ...
            if (client_count < MAX_CLIENT){
                ...
                char *register_ques = "Enter your \"client_id: client_name\": ";
                if (send(client, register_ques, strlen(register_ques), 0) == -1){
                    perror("send() failed");
                    continue;
                }
            }else{
                ...
                if (ret < 0){
                    ...
                    if (strcmp(clients[i].id, "") == 0 && strcmp(clients[i].name, "") == 0){
                        ...
                        if (ret == 2){
                            ...
                            char *register_ques = "Enter your \"client_id: client_name\": ";
                            if (send(clients[i].sockfd, register_ques, strlen(register_ques), 0) < 0){
                                perror("send() failed");
                                continue;
                            }
                            continue;
                        ...
                        for (int j = 0; j < client_count; j++){
                            if (j != i){
                                if (send(clients[j].sockfd, message, strlen(message), 0) == -1){
                                    perror("send() failed");
                                    continue;
                                }
                            }
